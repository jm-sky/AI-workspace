...
class TestRefreshAccessToken:
    ...
    async def test_refresh_access_token_preserves_tv_and_jti(self, auth_service: AuthService, mock_repository: AsyncMock, sample_user: User) -> None:
        ...
        entirely (or dropping `jti` from access). For any user whose
        tokenVersion isn't 0 (e.g. after a password reset), the very first
        refresh cycle would silently produce tokens that _verify_user_token
        rejects with 401 "Token has been revoked" — logging the user out
